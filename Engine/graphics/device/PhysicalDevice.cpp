...
PhysicalDevice::PhysicalDevice(const Share<Instance> &pInstance, const PhysicalDeviceCreateInfo& createInfo) : m_pInstance(pInstance) {
	uint32_t deviceCount = 0;
	vkEnumeratePhysicalDevices(m_pInstance->GetHandle(), &deviceCount, nullptr);
	LOG_ASSERT_INFO(deviceCount != 0, "Failed to find GPUs with kan support!");
	...
	LOG_DEBUG("PhysicalDevice Created.");
}

PhysicalDevice::~PhysicalDevice() {
}

bool PhysicalDevice::checkDeviceSupport(VkPhysicalDevice device, const std::vector<const char *>& extensions) {
...
u32 PhysicalDevice::GetQueueFamily(QueueType queueType) const {
...
std::optional<u32> PhysicalDevice::sGetQueueFamilyIndex(
...
std::optional<u32> PhysicalDevice::sGetQueueFamilyIndex(const std::vector<VkQueueFamilyProperties>& queueFamilies, VkQueueFlags with, VkQueueFlags without) {
...
void PhysicalDevice::acquireDepthFormatDetail() {
...
void PhysicalDevice::acquireQueueFamilyIndices() {
...
Share<PhysicalDevice> PhysicalDeviceBuilder::Build() const {
    return std::make_shared<PhysicalDevice>(m_pInstance, m_createInfo);
}

PhysicalDeviceBuilder& PhysicalDeviceBuilder::AddExtension(const char* ext) {
